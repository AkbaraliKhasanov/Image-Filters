...
class EditImageViewModel(private val editImageRepository: EditImageRepository) : ViewModel() {
    ...
    fun loadImageFilters(originalImage: Bitmap) {
        Coroutines.io {
            runCatching {
                ...
                editImageRepository.getImagesFilters(getPreviewImage(originalImage))
    ...
    private fun emitSaveFilteredImageUiState(
        ...
        val dateState = SaveFilteredImageDataState(isLoading, uri, error)
        saveFilteredImageDataState.postValue(dateState)
    }
